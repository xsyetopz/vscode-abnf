...
describe("grammar tokenizer", () => {
  ...
  test("does not tokenize syntax characters inside comments for every dialect", () => {
    ...
    for (const [dialect, text] of samples) {
      ...
      expect(
        tokens.some((token) => token.kind === "group" && token.text === "("),
      ).toBe(true);
  ...
  test("recognizes comments only outside quoted literals", () => {
    ...
    for (const [dialect, text] of samples) {
      ...
      expect(
        tokens.some(
          (token) => token.kind === "literal" && token.text.includes(";"),
        ),
      ).toBe(true);
      if (dialect === "ebnf") {
        expect(tokens.some((token) => token.kind === "comment")).toBe(false);
      } else {
        expect(tokens.filter((token) => token.kind === "comment")).toHaveLength(
          1,
        );
      }
    }
  ...
  test("does not let a semicolon inside an EBNF block comment hide later syntax", () => {
    ...
    expect(
      tokens.some(
        (token) => token.kind === "alternative" && token.text === "|",
      ),
    ).toBe(true);
    expect(
      tokens.some(
        (token) => token.kind === "reference" && token.text === "bar",
      ),
    ).toBe(true);
  });
  ...
  test("resets unterminated literal state at newlines before later comments", () => {
    const cases = [
      ...
      ["ebnf", 'root ::= "unterminated\n/* comment ()|*[]{} */ | bar'] as const,
    ];
  ...
  test("masks multiline W3C EBNF block comments", () => {
    ...
    expect(
      tokens.some(
        (token) => token.kind === "ruleName" && token.text === "hidden",
      ),
    ).toBe(false);
    expect(
      tokens.some(
